...
namespace Marcelsoft.NeuralNetwork
{
    public static class Storage
    {
        public static void Save(NeuralNetwork network, string path, string name)
        {
            ...
            {
                MatrixMarketWriter.WriteMatrix(Path.Combine(workingDir, $"layer{i}.weights.mtx"), network.Layers[i].Weights);
                MatrixMarketWriter.WriteMatrix(Path.Combine(workingDir, $"layer{i}.biases.mtx"), network.Layers[i].Biases);
            }
        ...
        public static NeuralNetwork Load(string path, string name)
        {
            ...
            {
                var weights = MatrixMarketReader.ReadMatrix<double>(Path.Combine(workingDir, $"layer{i}.weights.mtx"));
                var biases = MatrixMarketReader.ReadMatrix<double>(Path.Combine(workingDir, $"layer{i}.biases.mtx"));
